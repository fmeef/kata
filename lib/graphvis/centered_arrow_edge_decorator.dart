...
/// 线尾实心箭头
class CenteredArrowEdgeDecorator extends DefaultEdgeDecorator {
  ...
  @override
  void decorate(Edge edge, Canvas canvas, Paint paint) {
  ...
  void drawArrowOnEdge(Edge edge, Canvas canvas, Paint paint) {
    ...
    var yOffset = currentArrowWidth * edge.zoom / 16;
    var xOffset = this.xOffset;

    // 创建并填充箭头路径
    var path = Path()
      ..moveTo(tipPoint.x - xOffset, tipPoint.y - yOffset) // 箭头尖端
      ..lineTo(
        basePoint.x - xOffset + perp.x,
        basePoint.y + perp.y - yOffset,
      ) // 箭头左侧
      ..lineTo(
        basePoint.x - xOffset - perp.x,
        basePoint.y - perp.y - yOffset,
...
class LabelEdgeDecorator extends DefaultEdgeDecorator {
  Offset getCenterPoint(Vector2 vector1, Vector2 vector2) {
    return Offset((vector1.x + vector2.x) / 2, (vector1.y + vector2.y) / 2);
  }
  ...
  void decorate(Edge edge, Canvas canvas, Paint paint) {
    ...
    var paragraphConstraints = const ParagraphConstraints(
